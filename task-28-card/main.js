const pizza=[
    ...
    //   domRender(pizza)
      ...
      pizza.forEach(item => {
          ...
          card.className = 'card';
          card.innerHTML = `
          <img src="${item.img}" alt="${item.img}" onerror=this.src="https://static.vecteezy.com/system/resources/previews/026/766/386/non_2x/search-no-result-data-document-or-file-not-found-concept-illustration-flat-design-eps10-modern-graphic-element-for-landing-page-empty-state-ui-infographic-icon-vector.jpg">
          <div class="card-content">
          <h2 class="card-title">${item.name}</h2>
          <p class="card-desc">${item.dsc}</p>
          <p class="card-price">${item.price.toFixed(2)}</p>
          <p class="card-rate">⭐ ${item.rate}/5</p>
          <p class="card-country">📍 ${item.country}</p>
          <div>
          <button class="card-btn">add to fav</button>
...
//     let filteredList= data.filter(pizza=> pizza.name.toLowerCase().includes(e.target.value.toLowerCase()))
